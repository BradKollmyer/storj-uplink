...
- crates.io: `storj` 1.0.0 and its implementation crates. Public API is `storj::*` only; `storj-test` is unpublished.
...
| K3 | **Public crate name `storj`**, workspace of internal crates | `uplink` and `uplink-sys` are already taken on crates.io by the FFI project. `storj` is unused (crates.io keyword search, 2026-09-01). At 1.0 the facade and implementation crates are published; the facade re-exports the stable API. |
...
├── crates/
│   ├── storj/                       # published facade (public API)
│   ├── storj-access/                # grants, macaroons, encryption store; published as impl crate
...
At 1.0, `storj` and the implementation crates are on crates.io. Public API
remains `storj::*` (K17). `storj-test` stays unpublished. Grant-only tools use
`storj` re-exports.
...
| R12 | Noise protocol drift | Low | Local Go interoperability tests cover both cipher suites and maximum-size records; malformed/authentication failure tests cover the record layer |
...
Later: `object_store`. QUIC, Noise piece transfers, and local telemetry callbacks are implemented. Bucket notifications remain a non-goal.
