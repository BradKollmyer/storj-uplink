...
| `live.rs` | Production satellite smoke plus walkthrough (prefix list, range, copy, move, abort, small multipart, custom metadata, cross-bucket copy, `share()`, TCP vs Noise) | ignore + `STORJ_LIVE=1` (`STORJ_ACCESS` or `.env`) |
